"""
BraTS Dataset Download Helper Script with Robust Error Handling (v3.0 Final Release Gold Master)
"""
import argparse
import zipfile
import shutil
...
def is_patient_folder_complete(folder_path: Path) -> Tuple[bool, Optional[str]]:
    """
    統一完整性檢查邏輯：必須包含 4 模態 + Seg (v3.0 Final)
    """
    flair_files = list(folder_path.glob("*_flair.nii.gz"))
    if not flair_files:
        return False, None
    
    # v3.0 Final: 處理多個 flair 檔案的極端情況，優先選擇完整組合
    best_pid = None
...
def print_download_instructions():
    print("\n" + "="*70)
    print("📦 BraTS Dataset Download Instructions (v3.0 Final)")
    print("="*70 + "\n")
...
def auto_download_kaggle(data_dir: Path):
    print("\n🚀 Starting automatic download via Kaggle API (v3.0 Final)...\n")
    try:
        import kaggle
    except ImportError:
...
def main():
    parser = argparse.ArgumentParser(description='BraTS Dataset Download Helper (v3.0 Final)')
